import Navigation from "@/components/layout/navigation";

export default function Layout({ children }: { children: React.ReactNode }) {
    return (
        <div className="w-screen h-screen"
            style={{
                backgroundColor: '#727272',
                backgroundImage: `url("data:image/svg+xml;utf8,<svg width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'><line x1='0' y1='0' x2='20' y2='0' stroke='white' stroke-width='2' stroke-dasharray='2 2' stroke-opacity='0.15'/><line x1='0' y1='0' x2='0' y2='20' stroke='white' stroke-width='2' stroke-dasharray='2 2' stroke-opacity='0.15'/></svg>")`
            }}
        >
            <div className="w-7xl flex border-blue-500 mx-auto pt-10">

                <div className="relative pr-8 flex">
                    {/* 프로필 컨텐츠 사이 클립 버튼 */}
                    <>
                        <div className="absolute top-[150px] left-[308px] w-[24px] h-[10px] border-2 border-[#c9c9c9] rounded-3xl bg-white shadow-sm"
                            style={{
                                boxShadow: 'inset 11px 0 8px -8px rgba(0,0,0,0.4)'
                            }}
                        ></div>
                        <div className="absolute top-[185px] left-[308px] w-[24px] h-[10px] border-2 border-[#c9c9c9] rounded-3xl bg-white shadow-sm"
                            style={{
                                boxShadow: 'inset 11px 0 8px -8px rgba(0,0,0,0.4)'
                            }}
                        ></div>
                        <div className="absolute bottom-[185px] left-[308px] w-[24px] h-[10px] border-2 border-[#c9c9c9] rounded-3xl bg-white shadow-sm"
                            style={{
                                boxShadow: 'inset 11px 0 8px -8px rgba(0,0,0,0.4)'
                            }}
                        ></div>
                        <div className="absolute bottom-[150px] left-[308px] w-[24px] h-[10px] border-2 border-[#c9c9c9] rounded-3xl bg-white shadow-sm"
                            style={{
                                boxShadow: 'inset 11px 0 8px -8px rgba(0,0,0,0.4)'
                            }}
                        ></div>
                    </>

                    {/* 네비게이션 버튼 */}
                    <Navigation />

                    {children}

                </div>

                <div className="w-[240px]">
                    <div className="h-6 font-ginto text-[11px] text-white leading-6 bg-[#676566] text-center tracking-wide rounded-md">GO BACK HOME</div>
                    {/* 능력치 */}
                    <div className="bg-white px-4 py-4">
                        <div>
                            <div className="flex justify-center items-center gap-4">
                                <span className="w-[44px] text-[12px] text-gray-600 tracking-[-0.05em]">에로틱</span>
                                <div className="relative">
                                    <div className="w-20 h-[5px] bg-gray-200"></div>
                                    <div className="absolute top-0 left-0 w-8 h-[5px] bg-[#c6827f]"></div>
                                </div>
                                <div className="w-[18px] text-[12px] text-gray-600">0</div>
                            </div>
                            <div className="flex justify-center items-center gap-4">
                                <span className="w-[44px] text-[12px] text-gray-600 tracking-[-0.05em]">페이머스</span>
                                <div className="relative">
                                    <div className="w-20 h-[5px] bg-gray-200"></div>
                                    <div className="absolute top-0 left-0 w-8 h-[5px] bg-[#92ae7e]"></div>
                                </div>
                                <div className="w-[18px] text-[12px] text-gray-600">100</div>
                            </div>
                            <div className="flex justify-center items-center gap-4">
                                <span className="w-[44px] text-[12px] text-gray-600 tracking-[-0.05em]">프랜들리</span>
                                <div className="relative">
                                    <div className="w-20 h-[5px] bg-gray-200"></div>
                                    <div className="absolute top-0 left-0 w-8 h-[5px] bg-[#81a2b5]"></div>
                                </div>
                                <div className="w-[18px] text-[12px] text-gray-600">0</div>
                            </div>
                            <div className="flex justify-center items-center gap-4">
                                <span className="w-[44px] text-[12px] text-gray-600 tracking-[-0.05em]">카르마</span>
                                <div className="relative">
                                    <div className="w-20 h-[5px] bg-gray-200"></div>
                                    <div className="absolute top-0 left-0 w-8 h-[5px] bg-[#606163]"></div>
                                </div>
                                <div className="w-[18px] text-[12px] text-gray-600">0</div>
                            </div>
                            <div className="flex justify-center items-center gap-4">
                                <span className="w-[44px] text-[12px] text-gray-600 tracking-[-0.05em]">카인드</span>
                                <div className="relative">
                                    <div className="w-20 h-[5px] bg-gray-200"></div>
                                    <div className="absolute top-0 left-0 w-8 h-[5px] bg-[#cdbe59]"></div>
                                </div>
                                <div className="w-[18px] text-[12px] text-gray-600">0</div>
                            </div>

                        </div>
                        <hr className="border-gray-200" />

                        {/* 스크랩 즐겨찾기 */}
                        <div className="flex w-full bg-white">
                            <span className="w-1/2 text-[12px] text-gray-600">스크랩</span>
                            <span className="w-1/2 text-[12px] text-gray-600">즐겨찾기</span>
                        </div>
                        <hr className="border-gray-200" />

                        <div className="w-full bg-white">
                            <span className="w-1/2 text-[12px] text-gray-600">후원하기</span>
                        </div>
                    </div>

                </div>

            </div>
        </div>
    );
}