import "server-only";

import { cacheTag } from "next/cache";
import pool from "@/src/lib/db";
import {
  GET_BOARD_LIST,
  SELECT_BOARD_TOTAL_COUNT,
  GET_BOARD_CONTENT,
} from "./queries";
import type {
  BoardContentData,
  BoardData,
  BoardPaginationResult,
} from "./types";

/**
 * 게시글 목록을 페이지 단위로 조회합니다.
 * 한 페이지당 10개이며, `boardList` 캐시를 사용합니다.
 *
 * @param page - 1부터 시작하는 페이지 번호. 1보다 작으면 1로 보정됩니다.
 * @param type - 게시판 종류 id. `0`이면 전체 종류를 조회합니다.
 * @returns 게시글 목록과 전체 개수
 */
export async function getBoardList(
  page: number,
  type = 0
): Promise<BoardPaginationResult> {
  "use cache";

  cacheTag("boardList");

  const pageNum = Math.max(1, page);
  const limit = 10;
  const offset = (pageNum - 1) * limit;

  const [list, total] = await Promise.all([
    pool.query(GET_BOARD_LIST, [limit, offset, type]),
    pool.query(SELECT_BOARD_TOTAL_COUNT, [type]),
  ]);

  return {
    boards: list.rows as BoardData[],
    totalCount: Number(total.rows[0]?.total_count ?? 0),
  };
}

/**
 * 게시글 본문을 id로 조회합니다.
 *
 * @param id - 게시글 id
 * @returns 제목, 본문, 종류, 작성일 등 게시글 상세
 * @throws 게시글이 없거나 삭제된 경우
 */
export async function getBoardContent(id: number): Promise<BoardContentData> {
  "use cache";
  cacheTag(`boardContent-${id}`);

  const result = await pool.query(GET_BOARD_CONTENT, [id]);
  if (result.rows.length === 0) {
    throw new Error("게시글을 찾을 수 없습니다.");
  }

  return result.rows[0] as BoardContentData;
}