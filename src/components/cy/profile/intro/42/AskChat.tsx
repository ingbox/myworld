...
import {
  ...
  useResetAskMessages,
  useSendAskMessage,
} from "@/src/hooks/cy/profile/intro/42/use-42";

const AI_IMAGE = "/images/cy/profile/intro/42/jiseop-pixel.png";
...
export default function AskChat({
  ...
  const { data: usage } = useAskUsage();

  const remaining = usage?.remaining ?? null;
  const isOutOfQuota = remaining === 0;

  const { mutate, isPending, error } = useSendAskMessage();
  const { mutate: reset, isPending: isResetting } = useResetAskMessages();
